...
def draw_spiral(xx, yy, spin_dir):
    """Render a spiral galaxy with bifurcating arms and HII knots."""
    ...
    n_arms = int(torch.multinomial(torch.tensor([0.6, 0.4]), 1)) + 2
    ...
    for i in range(n_arms):
        ...
        fade_i = float(0.30 + 0.20 * torch.rand(1))
        width_i = float(0.30 + 0.20 * torch.rand(1))
        amp_i = float(0.9 + 0.7 * torch.rand(1))
        ...
        img += amp_i * brightness_asym * arm_fade_i * torch.exp((torch.cos(phi_i) - 1) / arm_width_i) * arm_mask
        ...
        if i in split_arm_idx:
            ...
            branch_amp = amp_i * float(0.55 + 0.3 * torch.rand(1))
            ...
            img += branch_amp * brightness_asym * branch_fade * torch.exp((torch.cos(phi_branch) - 1) / branch_width_r) * split_mask
    return img